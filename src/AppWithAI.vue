...
import { compareWithAI, generateAIInsights } from './utils/analyzerEnhanced'
...
import { setQuickMLConfig } from './utils/quickMLApi'
...
const quickMLToken = ref('')
const useAIAnalysis = ref(false)
...
const results = ref({
  android: { total: 0, text: 0, buttons: 0, inputs: 0, icons: 0 },
  ios: { total: 0, text: 0, buttons: 0, inputs: 0, icons: 0 },
  web: { total: 0, text: 0, buttons: 0, inputs: 0, icons: 0 },
  consistent: 0,
  inconsistent: 0,
  mapping: {
    text: [],
...
onMounted(() => {
  ...
  quickMLToken.value = localStorage.getItem('quickml_token') || ''
})
...
const saveQuickMLToken = () => {
  localStorage.setItem('quickml_token', quickMLToken.value)
  setQuickMLConfig({ authToken: `Zoho-oauthtoken ${quickMLToken.value}` })
  errorMessage.value = ''
}
...
const fetchDesigns = async () => {
  ...
  try {
    ...
    // Analyze
    loadingPlatform.value = 'Analyzing with AI'
    await analyzeDesigns()
...
const analyzeDesigns = async () => {
  try {
    if (useAIAnalysis.value && quickMLToken.value) {
      // Use AI-enhanced analysis
      loadingPlatform.value = 'AI Analysis in progress...'
      
      const aiResults = await compareWithAI(
        androidJson.value,
        iosJson.value,
        webJson.value,
        accessToken.value
      ...
      aiAnalysisResults.value = aiResults.aiAnalysis
      aiInsights.value = generateAIInsights(aiResults, aiResults.aiAnalysis)
    } else {
...
const generatePDF = () => {
  ...
  doc.text('PixelCheck AI-Enhanced Analysis Report', pageWidth / 2, yPos, { align: 'center' })
  yPos += 15

  doc.setFontSize(10)
  doc.setFont(undefined, 'normal')
  doc.text(`Generated: ${new Date().toLocaleString()}`, pageWidth / 2, yPos, { align: 'center' })
  doc.text(`Analysis Type: ${useAIAnalysis.value ? 'AI-Enhanced' : 'Traditional'}`, pageWidth / 2, yPos + 5, { align: 'center' })
  yPos += 20
  ...
  doc.setFontSize(11)
  doc.setFont(undefined, 'normal')
  doc.text(`Android: ${results.value.android.total} features`, 25, yPos)
  yPos += 7
  doc.text(`iOS: ${results.value.ios.total} features`, 25, yPos)
  yPos += 7
  doc.text(`Web: ${results.value.web.total} features`, 25, yPos)
  yPos += 10

  doc.text(`Consistent: ${results.value.consistent}`, 25, yPos)
  yPos += 7
  doc.text(`Inconsistent: ${results.value.inconsistent}`, 25, yPos)
  yPos += 15
  ...
  if (aiInsights.value.length > 0) {
    ...
    aiInsights.value.forEach(insight => {
      ...
      doc.text(`• ${insight.title}`, 25, yPos)
      yPos += 5
      doc.setFontSize(9)
...
const downloadPDF = () => {
  ...
  const analysisType = useAIAnalysis.value ? 'AI' : 'Standard'
  doc.save(`PixelCheck-${analysisType}-Report-${timestamp}.pdf`)
}
...
<template>
  <div class="container">
    <header class="header">
      <h1>🎨 PixelCheck AI - Intelligent Analysis</h1>
      <p class="subtitle">AI-Powered UI Design Consistency Validator with Semantic Matching</p>
    </header>
    ...
    <div class="input-section" v-if="!analysisComplete">
      ...
      <!-- QuickML AI Configuration -->
      <div class="ai-config-card">
        <div class="ai-header">
          <h3>🤖 AI-Enhanced Analysis (Optional)</h3>
          <label class="toggle">
            <input type="checkbox" v-model="useAIAnalysis" />
            <span class="toggle-slider"></span>
          </label>
        </div>
        <p class="help-text">
          Enable AI to detect semantically similar components (e.g., search bar vs search icon)
        </p>
        <div class="token-input-group" v-if="useAIAnalysis">
          <input
            v-model="quickMLToken"
            type="password"
            placeholder="Zoho Catalyst OAuth Token..."
            class="token-input"
            @blur="saveQuickMLToken"
          />
          <button @click="saveQuickMLToken" class="save-token-btn">💾 Save</button>
        </div>
        <div class="ai-info" v-if="useAIAnalysis">
          <p>✨ AI will identify functionally equivalent components across platforms</p>
          <p>🔍 Example: Search bar (Android) = Search icon (iOS)</p>
        </div>
      </div>
      ...
      <div class="loading-message" v-if="loading">
        ...
        <p>{{ loadingPlatform || 'Processing...' }}</p>
      ...
      >
        {{ loading ? '⏳ Analyzing...' : (useAIAnalysis ? '🤖 Fetch & Analyze with AI' : '🚀 Fetch & Analyze') }}
      </button>
    </div>
    ...
    <div class="results-section" v-if="analysisComplete">
      <div class="results-header">
        ...
        <div class="analysis-badge" v-if="useAIAnalysis">
      ...
      <!-- AI Insights -->
      <div class="ai-insights-section" v-if="aiInsights.length > 0">
        <h3>✨ AI Insights</h3>
        <div class="insight-card" v-for="(insight, index) in aiInsights" :key="index">
          <div class="insight-header">
            <span class="insight-icon">{{ insight.type === 'ai-discovery' ? '🔍' : '⚠️' }}</span>
            <h4>{{ insight.title }}</h4>
          </div>
      ...
      <div class="summary-card">
        ...
        <div class="summary-grid">
          <div class="summary-item">
            ...
            <span>{{ results.android.total }} features</span>
          </div>
          <div class="summary-item">
            <span class="platform-label">🍎 iOS:</span>
            <span>{{ results.ios.total }} features</span>
          </div>
          <div class="summary-item">
            <span class="platform-label">🌐 Web:</span>
            <span>{{ results.web.total }} features</span>
          </div>
        </div>
      ...
      <div class="mapping-section">
        ...
        <!-- Semantic Matches (AI) -->
        <div class="category-card ai-category" v-if="results.mapping.semanticMatches && results.mapping.semanticMatches.length > 0">
          <h4>🤖 AI-Detected Semantic Matches</h4>
          <p class="category-description">Components that serve the same function but look different</p>
          <div class="component-list">
            <div
              class="component-item ai-match"
              v-for="(item, index) in results.mapping.semanticMatches"
              :key="'semantic-' + index"
            >
              <span class="status-icon">🔍</span>
              <div class="match-details">
                <span class="component-name">{{ item.function }}</span>
                <div class="platform-implementations">
                  <span class="impl">Android: {{ item.android }}</span>
                  <span class="impl">iOS: {{ item.ios }}</span>
                  <span class="impl">Web: {{ item.web }}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="category-card" v-if="results.mapping.inputs && results.mapping.inputs.length > 0">
          <h4>📝 Input Fields</h4>
          <div class="component-list">
            <div
              class="component-item"
              v-for="item in results.mapping.inputs"
              :key="item.name"
            >
              <span class="status-icon">{{ item.icon }}</span>
              <span class="component-name">"{{ item.name }}"</span>
      ...
      <div class="action-buttons">
        <button class="action-btn download-btn" @click="downloadPDF">
          📥 Download {{ useAIAnalysis ? 'AI-Enhanced' : 'Standard' }} Report
        </button>
      </div>
    </div>
  </div>
...
<style scoped>
/* Base styles from previous component */
* {
  margin: 0;
  padding: 0;
...
.token-card h3, .ai-config-card h3 {
  color: #333;
  margin-bottom: 10px;
}
